...
- **LINE Voice Command**: Theater tradition - user says "line" during their turn to get whispered first 4 words of current line at 30% volume via browser TTS. Tracks hint usage in performance stats.
- **Hands-Free Mode**: Accessible from Settings drawer (Car icon). Full-screen dark overlay showing only current speaker/listening state, progress bar, and mic icon. Acquires Wake Lock to keep screen on. Auto-enables mic and starts playback on entry. Auto-restarts scene after 3-second pause on completion. Exit via X button.
