...
- **Motion**: 200ms ease-out transitions, subtle scale effects (0.98) on press, no playful animations. Page exit transitions (180ms fade-out with scale 0.98) in App.tsx. Three-line reader slide animation (250ms) on line advancement. Admin tab crossfade (200ms). Auth page staggered entry (scale-in logo, cascading fade-in-up). Settings drawer staggered item entry (50-400ms delays). Feature board and roadmap cards use hover shadow elevation (no scale on hover per guidelines).
