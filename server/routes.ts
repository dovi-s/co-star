...
MARY: You're kidding me.`;
  ...
  app.get("/api/stripe/subscription", async (req: any, res: Response) => {
    try {
      ...
      const buildLiveSubResponse = (liveSub: any) => {
      ...
      if (user.stripeCustomerId) {
        try {
          ...
          if (!liveSub) {
            ...
            liveSub = trialingSubs.data[0];
          }
          if (liveSub) {
            console.log(`[Stripe] Found sub via customer list for user ${userId}, syncing`);
            await db.update(users).set({
          ...
          console.warn(`[Stripe] Customer sub list failed for ${user.stripeCustomerId}:`, e.message);
        }
      }
